...
== Purpose
The purpose of this document is to describe the F1044 firmware's function and command set

== Functional Description
The F1044 firmware controls up to 8 7-segment displays. The displays are intended to be 4" displays.

NOTE: While the firmware can handle up to 8 displays as per hardware, it has only been tested for 6 displays, although
there is no reason it shouldn't work with 8.
...
* Ethernet or WiFi, telnet, port 23
* Ethernet or WiFi, ModBus, port 502 (IN PROGRESS)

All command frames sent to/from the device over serial or Telnet are in ASCII text mode, with either carriage-return (\r) or line-feed (\n) ending per frame.

If a command doesn't sent back a return value, an acknowledgment (ACK) may be sent back as the string "ok".
Any non-acknowledgments (NACK) may be sent as "error: X", where X is a human description of the error.
...
The following telnet commands (different than the commands on the firmware) are handled:
